...
class _NewAccountWidgetState extends BaseState<NewAccountPage> {
  ...
  final ValueNotifier<NewAccountStepEnum> _nextPreviousNotifier = ValueNotifier(
    NewAccountStepEnum.info,
  );
  ...
  void initState() {
    ...
    if (F.appFlavor == Flavor.app_live) {
      _initializeProductionData();
    } else {
      _initializeTestingData();
    }
  ...
  void _initializeTestingData() {
    widget._bloc.init(
      mobileNumberValue: "201272511442",
      countryId: 65,
      countryCode: "20",
    );
  }

  void _initializeProductionData() {
    widget._bloc.init(
            ...
            checkResponseStateWithButton(
              ...
              onSuccess: () {
                FirebaseAnalyticsUtil().logEvent(
                  ...
                  parameters: {
                    FirebaseAnalyticsKeyNames.org_id: event.response!.userId,
                  },
                );
